...
export function htmlToCustom(html: string): string {
    ...
    // 8. Lists
    // Process in reverse order to handle nested lists correctly
    const lis = Array.from(doc.querySelectorAll('li')).reverse()
    lis.forEach(el => el.outerHTML = `[li]${el.innerHTML}[/li]`)

    const uls = Array.from(doc.querySelectorAll('ul')).reverse()
    uls.forEach(el => el.outerHTML = `[ul]${el.innerHTML}[/ul]`)

    const ols = Array.from(doc.querySelectorAll('ol')).reverse()
    ols.forEach(el => el.outerHTML = `[ol]${el.innerHTML}[/ol]`)
