...
export function simulateBotAnswers(
  ...
  questions.forEach((question, index) => {
    ...
    if (isCorrect && question.correct_answer) {
       ...
       // Pick a wrong option from available options
...
// GameQuestion type from game-store (for duel mode)
...
// Union type for questions (supports both quiz and game modes)
type BotQuestion = QuizQuestion | GameQuestion;

/**
 * Simulates a single bot answer for the current question
 * Supports both QuizQuestion and GameQuestion types
 */
export function simulateSingleBotAnswer(
  ...
  onAnswer: (answer: string, timeTaken: number) => void
    ...
    // 2. Select answer
    let answer = "";
    // Use available options A, B, C, D
    const options = ['A', 'B', 'C', 'D'];
    
    // Handle both QuizQuestion (correct_answer) and GameQuestion (correctAnswer)
